...
router.get('/walmartlisting', function (req, res, next) {
    ...
    let paramArry = searchIndex.split("-");
    console.log(searchIndex);
    request('http://api.walmartlabs.com/v1/search?query='+paramArry[0]+'&format=json&' +
        'categoryId='+paramArry[1]+'&apiKey=ve3qcwh6dg4by7akjq7kxg97&sort=bestseller&numItems=25',
    ...
    router.get('/rakutenlisting', function (req, res, next) {
        ...
        request('http://popshops.com/v3/products.json?catalog=dp4rtmme6tbhugpv6i59yiqmr' +
            '&account=1j0h9zs42a8w062zk21xul727&keyword='+keyword+'&results_per_page=40&include_deals=1',
            function (error, response, body) {
            if (!error && response.statusCode == 200) {
                res.set('Content-Type', 'application/json');
                res.json(JSON.parse(body));
